...
async def backfill_chat_sessions(
    db: AsyncSession, time_window_minutes: int = 5, dry_run: bool = True
) -> dict:
    """
    ...
    for analysis in analyses:
        try:
            ...
            time_window_start = analysis.created_at - timedelta(
                minutes=time_window_minutes
            )
            time_window_end = analysis.created_at + timedelta(
                minutes=time_window_minutes
            )
            ...
            for session in potential_sessions:
                ...
                if first_message:
                    ...
                    if (
                        analysis_text_clean == message_content_clean
                        or analysis_text_clean in message_content_clean
                        or message_content_clean in analysis_text_clean
                    ):
            ...
            if matched_session:
                ...
                print(
                    f"  ✓ Matched analysis {analysis.id} to session {matched_session.id}"
                )
                stats["matched"] += 1
            else:
                print(
                    f"  ✗ No match found for analysis {analysis.id} (user: {analysis.user_id}, created: {analysis.created_at})"
                )
                stats["not_matched"] += 1
...
async def main():
    ...
    async for db in get_db():
        try:
            stats = await backfill_chat_sessions(
                db, time_window_minutes=5, dry_run=dry_run
            )
            ...
            print("=" * 60)
            print(
                f"Total analyses without chat_session_id: {stats['analyses_without_chat']}"
            )
